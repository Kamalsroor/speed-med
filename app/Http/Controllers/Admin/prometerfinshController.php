...
class prometerfinshController extends Controller
{
    public function index()
    {
      ...
      $MobilatEx = MobilatEx::where('active',0)->orderBy('id', 'DESC')->paginate(50);

        return view('admin.PrometerRequest.index3', compact('MobilatEx'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        for($i = 0 ; $i < count($request->Total); $i++){
            ...
            // if ($total_from !== $total_to) {
                //    return response()->json(['errors' => [$error_amount_total]], 422);
                $mobilats = Mobilat::orderBy('name')->get()->pluck('name', 'id');
                $Customers = Customers::get()->pluck('name', 'id')->toArray();
                // }
            if ($total !== $TotalS ) {
                return response()->json(['errors' => [$error_count_array]], 422);
    ...
    public function action2($id)
    {
        ...
        $MobilatEx = MobilatEx::where('id', $id)->update([
            ...
            $MobilatDetails = MobilatDetails::where('MobilatExID', $id)->update([
                'active' => $Entacc,
                ]);

        // delete transtion details
        return redirect()->route('prometerfinsh.index')->with('success', 'تم الموافقه علي الاودر بنجاح');
    }
